## QEUR21_MZPROT9: 閑話休題～バージョンアップ中 (トライアルE)

## ～　次からは、「閑話休題」なしで・・・　～

QEU:FOUNDER ; “今回で「3DMAZEの閑話休題」シリーズの最後にしましょう。ランダム命令であれば、メイズはほとんど進行しませんでした。”

![imageRL6-10-1](https://reinforce.github.io/images/imageRL6-10-1.jpg) 

D先生 : “強化学習はDQN with experience replay にしたんですよね。・・・、手法なんかはどうでもいいけど・・・。報酬はどうしたんですか？”

QEU：FOUNDER ; “いきなり目標達成が不可能だと思うので、**カリキュラム学習**にしました。そして、報酬は最後のターンにおけるゴール距離とminRT距離にしました。あとはPythonコードをドン！！長いよ・・・。”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step10E : [3D]RayCastと強化学習の併用
# PROGRAM NAME : demonstration_trialE_RayCast_Maze.py
# step10E : マニュアル操作版ですが、さらに量子化用の対応がされています
# step10E : カメラ画像の方向は、2DMazeの学習結果で決めています。
...
def calc_Boltzman(MazeXpos, MazeYpos, movablesMap, pos_lm):
    ...
    CTemp, arr_Gdists, arr_sminRTA = 500.0, [], []
    ...
    arr_probs = softmax(CTemp/(5*np.array(arr_Gdists)+np.array(arr_sminRTA)+1.0))
...
class DQN_Map_Solver3D:
    ...
    def __init__(self, stateMap_size, actionMap_size):
        ...
        self.epsilon    = 0.98 # randomness of choosing random actionMap or the best one
...
class Agent(object):
    ...
    def __init__(self, SCREEN):
        ...
        for iCnt_play in range(NUMS_EPISODES):
            ...
            # カリキュラム学習
            self.mx_lm        = [[0, 2],[15, 15]]   # ゴール　元は[26, 25]
            self.arr_lmG      = self.mx_lm[1]
    ...
    def get_episode(self, iCnt_play):
        ...
        for iCnt_turn in range(NUMS_TURNS):
            ...
            # ----------------
            if doneMap == True or iCnt_turn == NUMS_TURNS-1:
                # 最終報酬の計算
                if doneMap == True:
                    reward  = nominal_reward + 10000 - 500*int(end_distGoal) - 500*int(end_distRTA) - sum_bump
                else:
                    reward  = nominal_reward - 500*int(end_distGoal) - 500*int(end_distRTA) - sum_bump
            else:
                reward = nominal_reward - 0.1*int(end_distGoal) - 0.1*int(end_distRTA) - 0.1*sum_bump
            ...
            if next_stateMaze_chess[0] >= self.arr_lmG[0]-3 and next_stateMaze_chess[1] >= self.arr_lmG[1]-3:
                ...
                # 状態(STATE)表示
                if iCnt_turn%5 == 0:
                    print("iTurn:{}, Maze_chess: {}, Map_chess: {}, ViewAngle:{}, arr_RTDists:{}".format(iCnt_turn, stateMaze_chess, stateMap_chess, ViewAngle, arr_RTDists))
                # ----------------
                # 動画設定(4)
    ...
    def move_Maze_chess(self, stateMap_chess, actionMap, iCnt_turn):
        ...
        # ----------------
        # マップとメイズのXY座標[NEXT]
        next_MazeYpos = math.floor(next_stateMap_chess[0]/WIDE-0.5)   # メイズのY座標
        next_MazeXpos = math.floor(next_stateMap_chess[1]/WIDE-0.5)   # メイズのX座標
...
QEU：FOUNDER ; “まずは、パフォーマンスその１です。”

（パフォーマンス推移その１）

![imageRL6-10-2](https://reinforce.github.io/images/imageRL6-10-2.jpg) 

D先生 : “200ターンであれば、本当はもうちょっと歩いてほしいが、50回だけの学習であればしょうがないとしましょう。εが0.9以上ですからね。パフォーマンス指標はまだあるんですか？”

（パフォーマンス推移その２）

![imageRL6-10-3](https://reinforce.github.io/images/imageRL6-10-3.jpg) 

QEU：FOUNDER ; “もう一枚用意している（↑）よ・・・。B距離（限界）とC距離（限界）のグラフに注目してください。この値は、A距離（標準）その差を取っているんで、両方の値が同時にプラスになるということは「トラック（道路）の上」を走っているということなんです。”

D先生 ; “あれ？LOSS値の動きがちょっとおかしいと思います。”

QEU：FOUNDER ; “もうすでに何回もカリキュラム学習をしているので、初期のピーク値が採れているんです。すこしづつバグ修正、パラメタ変更しながらゴールに達成できるかをやってみましょう。”


## ～　まとめ　～

QEU:FOUNDER ： “いやいや、このニュースには（↓）びっくりしたわ・・・。この話がＪ国で大ニュースになっていないとはね・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/3Mgrdzyxlwg" ti-tle="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; en-crypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

D部長: “インターネットメディアでは、このマイナーメディアの記事しかありませんでした。”

![imageRL6-10-4](https://reinforce.github.io/images/imageRL6-10-4.jpg) 

QEU:FOUNDER ： “この記事はHKからだが、この話はもともとTWで大騒ぎになって発覚したモノです。これはいわゆる**「非常に悪質なスカウト詐欺」**です。高い報酬でスカウトされ、海外で働こうとした人が現地で誘拐され、人〇売買の対象になったという・・・。ほんとうに、J国の人は同様な被害がないのか？”

D部長: “私も、TWと聞いたときに驚きました。あそこの景気は悪くないので、わざわざ東南アジアで働こうとは思わないかと・・・。”

QEU:FOUNDER ; “要するに、世の中総じて景気が悪いですよ。だから、こういう恐ろしいことがはやるわけで・・・。私見だが、J国でも被害が絶対あると思うね・・・。”
