## QEUR21_MZPROT13: 転写学習をさせてみた（後半）

## ～　「転写学習」のカテゴリは？　～

D先生 : “一応は強化学習が進んでいます、いままで第2段階まで進みました。これから**「第3段階」**に行きたいと思います。なんか、今度は**「新しいコトをしたい」**とおっしゃっていましたね？”

**(第2段階-A)**

![imageRL6-14-1](https://reinforce.github.io/images/imageRL6-14-1.jpg) 

**(第2段階-B)**

![imageRL6-14-2](https://reinforce.github.io/images/imageRL6-14-2.jpg) 

**(第2段階-C)**

![imageRL6-14-3](https://reinforce.github.io/images/imageRL6-14-3.jpg) 

QEU:FOUNDER ; “**Boltzmann選択において、温度を段階的に変化できるようにしたかった**んです。プログラムは、「このあたり（↓）」ですね。”

![imageRL6-14-4](https://reinforce.github.io/images/imageRL6-14-4.jpg) 

D先生 : “それが、いったい何の役に・・・？

![imageRL6-14-5](https://reinforce.github.io/images/imageRL6-14-5.jpg) 

QEU：FOUNDER ; “我々の**「転写学習」は、「教師あり学習」と「強化学習」の中間にある概念**です。どちらに傾いても良くない・・・。目標ボルツマン温度(C_Target)を適切に調整すれば、**「ちょうどよい塩梅が実現」**できるんです。”

![imageRL6-14-6](https://reinforce.github.io/images/imageRL6-14-6.jpg) 

D先生 : “温度は段階的に変えるんですね。”

QEU：FOUNDER ; “詳しい説明は、プログラムをドンのあとで！！”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step13 : [3D]RayCastと強化学習の併用
# PROGRAM NAME : Reinforcement_RunD_RayCast_Maze.py
# step13 : カメラ画像の方向は、2DMazeの学習結果で決めています。量子化用の対応がされています
# 注意： 簡単な強化学習をします、実行版C段階です(RUN_D)。boltzmann選択にさらに手直しがなされています。
# ----------------
import cv2
import numpy as np
import pandas as pd
...
class Agent(object):
    ...
    def __init__(self, SCREEN):
        ...
        for iCnt_play in range(NUMS_EPISODES):
            ...
            # カリキュラム学習
            self.mx_lm[1]   = [18, 20]    # ゴール　元は[26, 25]   [9, 8](initial) -> [12, 8](second) -> [17, 8] -> [18, 17] -> [18, 20]
            self.arr_lmG    = self.mx_lm[1]
    ...
    def get_episode(self, iCnt_play):
        ...
        for iCnt_turn in range(NUMS_TURNS):
            ...
            # ----------------
            # 次の行動(ACTION)を発行する
            acTypeMap, actionMap, QvalueMap = Dql3DMap_solver.choose_actionMap(stateMap_chess, stateMap, movablesMap, pos_lmG, self.level_transform)
    ...
    def show_graph(self):
        ...
        ax2.scatter(self.arr_iplay, arr_success, label="GAME", color="red")
        ax2.grid(True)
        ax2.legend(loc='best')
        # -----
...
QEU：FOUNDER ; “それでは、第三段階の実行結果を見てみましょう。”

**(第3段階-A)**

![imageRL6-14-7](https://reinforce.github.io/images/imageRL6-14-7.jpg) 

**(第3段階-B)**

![imageRL6-14-8](https://reinforce.github.io/images/imageRL6-14-8.jpg) 

**(第3段階-C)**

![imageRL6-14-9](https://reinforce.github.io/images/imageRL6-14-9.jpg) 

D先生 : “εが4回も復帰していますね。本当に、そんなに必要なんでしょうか？”

QEU：FOUNDER ; “パフォーマンスを見れば、復帰は2回ぐらいで十分でしょう。むしろ、**εの減衰率(e_decay)を調整して情報量を多くしたほうがいい**でしょうね。また、学習収束の評価についてですが、「教師あり学習」を参考にしたほうがいいと思うよ。以下のFast.AIの記事を参考にしてみましょう。MNISTに関するトピックになります。”

![imageRL6-14-10](https://reinforce.github.io/images/imageRL6-14-10.jpg) 

QEU：FOUNDER ; “当たり前だが、ディープラーニングは「回帰（教師あり学習）」に使えます。たとえ教師情報をインプットしても、ディープラーニング関数がフィットするには、かなりの繰り返し計算が必要なんですよ。”

![imageRL6-14-11](https://reinforce.github.io/images/imageRL6-14-11.jpg) 

D先生 : “じゃあ、学習収束を評価するにはどうすればいいんですか？”

QEU：FOUNDER ; “定期的に**「出来栄えを見る」**ことによって評価するしかないと思いますよ。次に行きましょう。”


## ～　まとめ　～

QEU:FOUNDER ： “いやいや・・・。これは、ひどい状況だ・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/rheV-n3dgW8" title="YouTube video player" frameborder="0" allow="accelerometer; au-toplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allow-fullscreen></iframe>

D部長: “例の風水の先生がいうところの、**「カタストルフィ（壊滅的）」っ**ていう奴・・・。”
...
QEU:FOUNDER ： “小生もはやり病がこれほどひどくなる（世界一！）とは全く予想していませんでした。まさに、百発百中！！この先生が首相になったほうがいいかもしれない・・・(笑)。”
