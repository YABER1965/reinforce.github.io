## QEUR21_MZPROT12: 転写学習をさせてみた（前半）

## ～　「大改造」は、まだ終わらない・・・　～


D先生 : “強化学習が、なぜか**「（関数）転写学習」に改名**されてしまいました（笑）。本来は、カリキュラム学習が第3段階にいくはずなんですが、今回こそはすんなりとイケますか？”

**（パフォーマンス推移A、第二段階-STEP3）**

![imageRL6-13-1](https://reinforce.github.io/images/imageRL6-13-1.jpg) 

**（パフォーマンス推移B、第二段階-STEP3）**

![imageRL6-13-2](https://reinforce.github.io/images/imageRL6-13-2.jpg) 

QEU:FOUNDER ; “またまたプログラムを変えました（笑）。ε-Greedyでは**「関数の転写」が難しい**ことがわかりました。上記の例（↑）では、学習の後半において、コマが別の場所に収束されました。これは学習の初期（exploration）の段階で当該領域が十分開拓（exploration）されていないため、学習後期の活用(exploitation)段階で発生したエラーであろうと思います。・・・ただし、我々からすればね。そもそも・・・。そんな場所は「学習する必要があるのか？」と・・・。”

D先生 : “まあ、現実に使わなければ、そもそも（学習する必要は）ないですね・・・？

![imageRL6-13-3](https://reinforce.github.io/images/imageRL6-13-3.jpg) 

QEU：FOUNDER ; “**「我々のいわゆる転写学習」のコンセプトではボルツマン選択で使う「単位空間」の情報が最も正しいんですよ。**bellman方程式でQmax計算した結果が正しいと考えるような「普通のε-Greedy法」とは考え方が逆なんです。だから、解析用のプログラムも若干変わります。”

D先生 : “どう（コードが）変わるんですか？”

QEU：FOUNDER ; “妙に説明するより、実際に実行して結果をみたほうがいいです。プログラムをドン！！”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step12 : [3D]RayCastと強化学習の併用
# PROGRAM NAME : Reinforcement_RunC_RayCast_Maze.py
# step12 : カメラ画像の方向は、2DMazeの学習結果で決めています。量子化用の対応がされています
# 注意： 簡単な強化学習をします、実行版C段階です(RUN_C)。ε-Greedyに手直しがなされています。
# ----------------
import cv2
import numpy as np
import pandas as pd
...
class Agent(object):
    ...
    def __init__(self, SCREEN):
        ...
        for iCnt_play in range(NUMS_EPISODES):
            ...
            # カリキュラム学習
            self.mx_lm[1]   = [18, 10]    # ゴール　元は[26, 25]   [9, 8](initial) -> [12, 8](second) -> [17, 8] -> [20, 17] -> [20, 22]
            self.arr_lmG    = self.mx_lm[1]
...
if __name__ == '__main__':
    ...
    comment_input_3Dmodel = "initialC"
    ...
    comment_output_3Dmodel = "secondC"
...
D先生 : “・・・なるほどね。**Exploitation（活用）に異常が発生したら、強制的にExploration（探索）の段階に戻す**と・・・。我々はJeremy Howardではないので、このアタマで考えられる工夫はこの程度かと・・・（笑）。”

 **(第1段階-A)**

![imageRL6-13-4](https://reinforce.github.io/images/imageRL6-13-4.jpg) 

**(第1段階-B)**

![imageRL6-13-5](https://reinforce.github.io/images/imageRL6-13-5.jpg) 

**(第1段階-C)**

![imageRL6-13-6](https://reinforce.github.io/images/imageRL6-13-6.jpg) 

QEU：FOUNDER ; “このやり方を使えば、転写学習の運用はずいぶん簡単になります。ε-Greedyでいうところの学習収束を無視するので、ステップを踏んで徐々に複雑な関数にトライする必要はないです。いきなりやっても、多分・・・、やれるでしょう。さらには、ε-Greedyの減衰率を大きくすることができます。我々のやり方ではεの大きさは、それほど重大な概念でもなくなったし・・・。さらには、ディープラーニング関数の学習率(lr)を大きくとれますよ”

D先生 : “つまり、**学習計算がかなり速くなる**と・・・。”

QEU：FOUNDER ; “実際のところ、**今までよりも全然速い**です・・・。それでは、第2段階の結果をみてみましょう。”

**(第2段階-A)**

![imageRL6-13-7](https://reinforce.github.io/images/imageRL6-13-7.jpg) 

**(第2段階-B)**

![imageRL6-13-8](https://reinforce.github.io/images/imageRL6-13-8.jpg) 

**(第2段階-C)**

![imageRL6-13-9](https://reinforce.github.io/images/imageRL6-13-9.jpg) 

D先生 : “いやぁ・・・、2回εが復帰されましたか・・・。それにしても、いきなり第二段階として、こんなに遠い場所で学習できるんですか？”

QEU：FOUNDER ; “それがメリットだけどね。”

D先生 : “それにしても、**学習収束の定義がわかりにくくなりました**ね。”

QEU：FOUNDER ; “つまり、本手法は**「まだ未完成デス。次回につづく」**ということ・・・(笑)。”


## ～　まとめ　～

C部長： “最近、イケメンバトルが低調ですね・・・。”

![imageRL6-13-10](https://reinforce.github.io/images/imageRL6-13-10.jpg) 

D部長: “選挙が終わったので、低調になるのでしょうがないでしょう。例の、あの別件で盛り上がっていますからね。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/4o7hqUqJ4Gc" ti-tle="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; en-crypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ： “あの「界」自体に盛り上がりがない・・・(笑)。”

D部長: “この人（↓）に期待しましょう・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/eFcb8DY8Lvk" ti-tle="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; en-crypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
