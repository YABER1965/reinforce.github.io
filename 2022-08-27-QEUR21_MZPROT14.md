## QEUR21_MZPROT14 :  出来栄えを測る

## ～　測ったからできる！方向転換！！　～

### ・・・　前回のつづきです　・・・

D先生 : “学習収束を評価するにはどうすればいいんですか？”

QEU：FOUNDER ; “定期的に**「出来栄えを見る」ことによって評価するしかない**と思いますよ。こんな評価方法はどうかな？”

![imageRL6-15-1](https://reinforce.github.io/images/imageRL6-15-1.jpg) 

QEU：FOUNDER ; “**Boltzmann選択した結果とQmax選択した結果を比較し、その一致量をパフォーマンスとする**という・・・。”

D先生 : “つまり、Boltzmann選択の結果が正しいという前提ですね。”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step14C : [3D]RayCastと強化学習の併用
# PROGRAM NAME : RouteTrace_CHECK_RayCast_Maze.py
# step14C : カメラ画像の方向は、2DMazeの学習結果で決めています。量子化用の対応がされています
# 注意： 学習結果でルートの軌跡を描き、パフォーマンス正解ポイントを計測します。
# ----------------
import cv2
import numpy as np
import pandas as pd
...
#=================================================
# ボルツマン選択による出現確率の調整
def calc_Boltzman(MazeXpos, MazeYpos, movablesMap, pos_lm, level_transform):

    # ボルツマン選択レベルの目標値
    #arr_CTarget = [15.0, 20.0, 25.0, 30.0, 35.0, 40.0]    # ボルツマン選択レベルの目標値
    arr_CTarget = [40.0]    # ボルツマン選択レベルの目標値
    # ------
    C_Temp, arr_Gdists, arr_sminRTA = 100.0, [], []
    ...
    C_Target    = arr_CTarget[level_transform]   # ボルツマン選択レベルの目標値
    arr_values  = C_Temp/(0.03*arr_Gdists**3+10*arr_sminRTA**2+1.0)
    mean_values = np.mean(arr_values)
    arr_probs   = softmax(C_Target*arr_values/mean_values)
...
#=================================================
# 復帰イプシロン値の見直し
def fn_review_epsilon(arr_victory, arr_epsilon):

    # ゲームオーバー数を数える
    sum_victory = 0
    for i in range(len(arr_victory)):
        if arr_victory[i] == "success":  # 画像の大きさ
            sum_victory = sum_victory + 1
    sum_victory = int(0.2*sum_victory)
            
    # 復帰イプシロン値を見直す
    review_epsilon  = arr_epsilon[sum_victory-1]
    flg_epsilon     = True

    return review_epsilon, flg_epsilon
...
class Net3D(torch.nn.Module):
    def __init__(self):
        ...
        self.fc1 = torch.nn.Linear(dim_input3D, 256)
        self.fc1.weight.data.normal_(0, 0.1)
        self.fc2 = torch.nn.Linear(256, 256)
        self.fc2.weight.data.normal_(0, 0.1)
        self.fc3 = torch.nn.Linear(256, 256)
        self.fc3.weight.data.normal_(0, 0.1)
        self.fc4 = torch.nn.Linear(256, dim_output3D)
        self.fc4.weight.data.normal_(0, 0.1)
...
class DQN_Map_Solver3D:
    ...
    def __init__(self, stateMap_size, actionMap_size):
        ...
        #self.e_decay    = 0.992     # epsilon decay rate
        #self.e_min      = 0.01  # minimum rate of epsilon
        #self.learning_rate  = 0.005     # learning rate of neural network
        # -------------------------
        # crate instance for input
    ...
    def choose_actionMap(self, stateMap_chess, stateMap, movablesMap, pos_lmG, level_transform):

        acTypeMap, QvalueMap, arr_Qvalue, num_matching = "random", -0.001, [-99, -99, -99, -99], 1
        # マップの座標をメイズのXY座標に変換する
        MazeYpos = math.floor(stateMap_chess[0]/WIDE)   # メイズのY座標
        MazeXpos = math.floor(stateMap_chess[1]/WIDE)   # メイズのX座標
        ...
        if MazeYpos == pos_lmG[0] and MazeXpos == pos_lmG[1]:
            ...
            if len(movablesMap) > 0:
                # ボルツマン選択による出現確率の調整
                arr_probs = calc_Boltzman(MazeXpos, MazeYpos, movablesMap, pos_lmG, level_transform)
        ...
        acTypeMap_c, actionMap_c, QvalueMap_c, arr_Qvalue_c = self.choose_best_actionMap(stateMap, movablesMap)

        # ---------------------------
        # matching and measuring 
        QvalueMap   = QvalueMap_c 
        arr_Qvalue  = arr_Qvalue_c
        if actionMap != actionMap_c:
            num_matching = 0

        return acTypeMap, actionMap, QvalueMap, arr_Qvalue, num_matching
    ...
    def choose_best_actionMap(self, stateMap, movablesMap):
        ...
        return acTypeMap, actionMap, QvalueMap, y_pred
...
class Agent(object):
    ...
    def __init__(self, SCREEN):
        ...
        # -------------------------
        # 見直し用イプシロン値は設定されているか
        flg_epsilon        = False     # 転写フラグ
        review_epsilon     = -0.99     # 復帰するイプシロン値
        self.level_transform  = 0     # 転写レベル
        ...
        iCnt_play = 1
        # カリキュラム学習
        self.mx_lm[1]   = [22, 22]    # ゴール　元は[26, 25]   [9, 8](initial) -> [12, 8](second) -> [17, 8] -> [18, 17] -> [18, 20]
        self.arr_lmG    = self.mx_lm[1]

        # エピソードを実行する
        maxturn, maxbump, maxscore, flag_goal, end_distGoal, end_distRTA, sum_matching = self.get_episode(iCnt_play)
        # ========================================
        ...
        # 学習結果の出力
        #if iCnt_play % 2 == 0:
        print("iPlay:{}, maxturn:{}, maxbump:{}, maxscore:{}, sum_matching:{}, flag_goal:{}, end_distGoal:{}, end_distRTA:{}".format(iCnt_play, maxturn, maxbump, maxscore, sum_matching, flag_goal, end_distGoal, end_distRTA))
        # -----------------
        # 記録用パラメタ類(プレイベース)
        ...
        if iCnt_play > 10:
            # 三回連続してゲームに失敗したら見直し
            if self.arr_victory[iCnt_play] != "success" and self.arr_victory[iCnt_play-1] != "success":
                if flg_epsilon == False:
                    # 復帰イプシロン値の設定
                    review_epsilon, flg_epsilon = fn_review_epsilon(self.arr_victory, self.arr_epsilon)
                # イプシロン値の見直し
                Dql3DMap_solver.epsilon = review_epsilon
                # 転写レベルの見直し
                if self.level_transform < 6:
                    self.level_transform    = self.level_transform + 1
        ...
        #ax6.set_ylabel('maxturn')
        #ax6.plot(self.arr_iplay, self.arr_maxturn, label="ROUTE", color="blue")
        #ax6.grid(True)
        #ax6.legend(loc='best')
        #fig3.tight_layout()
        #fig3.savefig("./AAC_img.png")
        #plt.show()
    ...
    def get_episode(self, iCnt_play):
        ...
        for iCnt_turn in range(NUMS_TURNS):
            ...
            # ----------------
            # 次の行動(ACTION)を発行する
            acTypeMap, actionMap, QvalueMap, arr_Qvalue, num_matching = Dql3DMap_solver.choose_actionMap(stateMap_chess, stateMap, movablesMap, pos_lmG, self.level_transform)
            # 報酬(nominal_reward)を発行する
            nominal_reward, doneMap = maze_field.get_valMaze(stateMaze_chess)
            # 回転と前進に翻訳してコマを動かす
            ViewAngle, ViewDirection, next_stateMaze_chess, next_stateMaze, next_stateMap_chess, next_stateMap, next_arr_RTDists, flg_canGo = self.move_Maze_chess(stateMap_chess, actionMap, iCnt_turn+1)
            if flg_canGo == False:
                sum_bump = sum_bump + 1
            # ----------------
            # 移動可能な命令群
            if stateMaze_chess == self.start_point:
                ...
                next_movablesMap = maze_field.get_ActionMaps(next_stateMap_chess, self.arr_MPODR)
            # ----------------
            if len(next_movablesMap) == 0:
                # movableに異常が発生しました(ABNORMAL)
                nominal_reward, reward, doneMap = -5000, -5000, False
                next_movablesMap  = ["DOWN", ]
                stateMaze_chess   = self.start_point
                stateMap_chess    = [round(pos_lmS[0]*WIDE+WIDE/2), round(pos_lmS[1]*WIDE+WIDE/2)]
                acTypeMap, actionMap, QvalueMap = "abnormal", "DOWN", -0.01
                # 2種類の状態(State)と距離(Dist)群を生成する
                ViewAngle, ViewDirection, stateMaze, stateMap, arr_RTDists = self.create_states(iCnt_turn, stateMaze_chess, stateMap_chess)
                # 回転と前進に翻訳してコマを動かす
                ViewAngle, ViewDirection, next_stateMaze_chess, next_stateMaze, next_stateMap_chess, next_stateMap, next_arr_RTDists, flg_canGo = self.move_Maze_chess(stateMap_chess, actionMap, iCnt_turn+1)
                print("sudden break - back to startpoint, reward:{}".format(reward))
            # ----------------
            # 記録用パラメタ類(ターンベース)
            ...
            # ----------------
            if next_stateMaze_chess[0] >= self.arr_lmG[0]-2 and next_stateMaze_chess[1] >= self.arr_lmG[1]-2:
                # ゴールに到達しました(NORMAL)
                flag_goal   = "success"
                doneMap     = True
                ...
                # ----------------
                # 最終報酬の計算
                reward  = nominal_reward + 15000 - 1000*int(end_distRTA) - sum_bump - 3*iCnt_turn
                ...
                if iCnt_turn == NUMS_TURNS-1:
                    ...
                    doneMap     = True
                    # 最終報酬の計算
                    reward  = nominal_reward - 1000*int(end_distRTA) - sum_bump
                    print("sorry! no enoughgame time, reward:{}".format(reward))
                else:
                    # 中間段階
                    reward  = nominal_reward - 0.7*int(end_distRTA) - 0.1*sum_bump
                # ----------------
                # [Map]データを積み上げる
                ...
                # 状態(STATE)表示
                #if iCnt_turn%5 == 0:
                print("iTurn:{}, Maze_ch:{}, Map_ch:{}, ViewAng:{}, Qvalues:{}, reward:{}, RTDists:{}".format(iCnt_turn, stateMaze_chess, stateMap_chess, ViewAngle, arr_Qvalue, reward, arr_RTDists))
                # ----------------
                # カウントアップ
                ...
                sum_matching = sum_matching + num_matching
                sum_score    = sum_score + nominal_reward
        ...
        return maxturn, maxbump, maxscore, flag_goal, end_distGoal, end_distRTA, sum_matching
    ...
    def show_graph(self):
        ...
        # ----------------
        # ゲームの成功を可視化する
        arr_success   = []  # victory    勝利したか
        for i in range(len(self.arr_victory)):
            if self.arr_victory[i] != "success":  # victory    勝利したか
                arr_success.append(0.2)
            else:
                arr_success.append(0.5)
        # ----------------
        # 1ターンのみ、軌跡グラフを表示する
        # 壁の部分（赤色）を表示する
        mx_draw = []
...
QEU：FOUNDER ; “Boltzmann選択した結果では正しいルートがでるんだよね・・・。”

![imageRL6-15-2](https://reinforce.github.io/images/imageRL6-15-2.jpg) 

QEU：FOUNDER ; “パフォーマンスは以下のように測れます・・・。「maxturn」と「sum_matching」に注目してください。”

![imageRL6-15-3](https://reinforce.github.io/images/imageRL6-15-3.jpg) 

D先生 : “**「一致率」というのはsum_matching/maxturnということ**になるんですかねぇ・・・。・・・じゃあ、ぜんぜんダメじゃないですか・・・。一致率が50％程度しかありません。

QEU：FOUNDER ; “**目標ボルツマン温度(C_Target)は40に設定している**ので、やや高め・・・。でも、結局のところ乱数設定になるので、この一致率は最高で80％程度しかならないんじゃないかなふぁ・・・。”

![imageRL6-15-4](https://reinforce.github.io/images/imageRL6-15-4.jpg) 

D先生 : “それでも、なんとか評価指標にはなるでしょう。”

QEU:FOUNDER ; “この結果を鑑みてもう一回学習してみました。ちょっとだけマシになるかなと・・・。どのようにしたのかは質問しないで・・・。でも、うまく行ったら聞いてもいいよ（笑）。”

D先生 : “じゃあ、学習結果のパフォーマンスのみを見せてくださいね。

QEU：FOUNDER ; “それでは結果をドン！これを「第4段階」と命名しましょう。”

**(第4段階-A)**

![imageRL6-15-5](https://reinforce.github.io/images/imageRL6-15-5.jpg) 

**(第4段階-B)**

![imageRL6-15-6](https://reinforce.github.io/images/imageRL6-15-6.jpg) 

**(第4段階-C)**

![imageRL6-15-7](https://reinforce.github.io/images/imageRL6-15-7.jpg) 

D先生 : “FOUNDERが今回において目標とするパフォーマンス（一致率）は？”

QEU：FOUNDER ; “65％から70％ぐらいかなぁ・・・。それでは結果をドン！”

![imageRL6-15-8](https://reinforce.github.io/images/imageRL6-15-8.jpg) 

D先生 : “ずいぶん遠いところまで歩きましたね。”

QEU：FOUNDER ; “ついでだからね。そしてパフォーマンスはこうなりました。”

![imageRL6-15-9](https://reinforce.github.io/images/imageRL6-15-9.jpg) 

D先生 : “あ～あ・・・。まえより**パフォーマンスが悪くなりました**。”

QEU：FOUNDER ; “D先生、小生が第4段階でなにをやったか、知りたい？”

D先生 : “いや、いいです（笑）。”

![imageRL6-15-10](https://reinforce.github.io/images/imageRL6-15-10.jpg) 

QEU：FOUNDER ; “我々が提唱する**「転写学習」は教師あり学習と強化学習の中間だ**と説明しました。多分、そのバランスを見直すべきなんでしょうね。”

D先生 : “評価ができたからこそ、方針変更ができました。これはめでたいことです。”


## ～　まとめ　～

C部長: “選挙が終わったので、低調になるのでしょうがないでしょう。例の、あの別件で盛り上がっていますからね。”

![imageRL6-15-11](https://reinforce.github.io/images/imageRL6-15-11.jpg) 
 
C部長: “アゲるとすると、おそらくコレ（↓）かなぁ・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/kz2YgjxCoUM" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ： “だれ？”

D部長: “私には**「おきなわの人」**ということしかわかりません・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/wVY7MNII4-o" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ： “**「おきなわ」にはすごいポテンシャルがあります**。もし、世の中が良い方向にかわるとすると、「多分ここからかな？」と・・・。”
