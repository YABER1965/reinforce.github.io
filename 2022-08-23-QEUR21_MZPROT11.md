## QEUR21_MZPROT10: 一応、走行ルートを学習させてみた(カリキュラム学習-第2段階をやり直し)

## ～　「強化学習」をやっているわけではない・・・　～

D先生 : “一応は強化学習が進んでいます、ここまで第2段階まで進みました。これから「第3段階」に行きたいと思います。あれ？「第4段階」はやらないんですか？”

**（パフォーマンス推移A-STEP3）**

![imageRL6-12-1](https://reinforce.github.io/images/imageRL6-12-1.jpg) 

**（第3段階の定義）**

![imageRL6-12-2](https://reinforce.github.io/images/imageRL6-12-2.jpg) 

QEU:FOUNDER ; “「あることをやってみるため」にこのプロジェクトをやっています。そのために第四段階は要るかなあ・・・？強化学習はなにしろ計算時間がベラボーにかかるので、ゲームの規模を大きくしたくないんですよ。”

D先生 : “**「あることをやってみるため」**・・・？

![imageRL6-12-3](https://reinforce.github.io/images/imageRL6-12-3.jpg) 

QEU：FOUNDER ; “今回のプロジェクトの目的は二つあります。一つは、第3段階が無事に終わってからやってみましょう。もう一つの目的がより重要です。我々がやっているのは「強化学習じゃない」んですよ、「ボルツマン選択をつかった関数転写」です・・・。”

D先生 : “ふ～ん・・・、（関数）転写ねえ・・・。射出成形やプレスじゃないんだから・・・。”

![imageRL6-12-4](https://reinforce.github.io/images/imageRL6-12-4.jpg) 

QEU：FOUNDER ; “そうそう・・・、それ！**プレスだよ**！！今回のスキームのアイデアはもともと順送（プログレッシブ）プレスからきているんです。RT距離に使った単位空間の情報をディープラーニングの関数に転写しているんです。複雑な関数なので、いきなり全部の単位空間の情報を転写することはできません。だから、プログレッシブに数段階に分けて徐々に転写しているんです。”

D先生 : “転写と聞くと、AIに疎い製造業のオッサンでも少しは興味をもつかねぇ・・・(笑)。あ～ぁ・・・、オッサンどもは自分の理解できない新しいモノがでると、すぐに否定（保身）にいくからねぇ・・・。一方、我々の説明は普通に強化学習を理解しているマトモな人から見れば「ハァ？」でしょうね・・・（笑）。”

![imageRL6-12-5](https://reinforce.github.io/images/imageRL6-12-5.jpg) 

QEU：FOUNDER ; “じゃあ、強化学習の立場から説明しましょうか・・・。強化学習・・・、というか機械学習の基本的な考え方は**「Exploitation and Exploration(探索と活用)」**です。近年の強化学習の発展はExploitation側の研究の成果です。一方のExplorationの方は、基本はε-Greedyであり、それほど手法は多くありません。今回の我々のやり方は、Exploration（活用）側の工夫なんです。エンジニアが設定した最適条件を「単位空間」とし、QEUシステムが考案したminRT距離とBoltzmann選択を使って、単位空間の情報をディープラーニング関数に転写をします。我々のやり方は厳密な意味では強化学習とは言えないんじゃないかな？”

D先生 : “我々のやり方は、別に最適スコアを得るためのやり方ではありません。**エンジニアの考える単位空間に基づいた最適ルートを効率的に求める**ものです。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/8SF_h3xF3cE" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU：FOUNDER ; “まあ、要するに我々は大したことをやっていないというわけ・・・（笑）。Jeremy Howardぐらいの能力があれば、「転移学習(transfer learning)」でやってみたいテーマなんだがね・・・。”


## ～　またやり直し？またコードを変えた？　～

QEU：FOUNDER ; “第二段階をやり直しました。”

D先生 : “またプログラムを変えたんですか？”

QEU：FOUNDER ; “ある例外処理が学習の収束を阻害していたので、ちょっとやりかたを改造しました。あと、ディープラーニングの関数定義が簡単（128x3）だったので、より複雑に(256x3)しました。それでは修正したコードをドン！！”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step11B : [3D]RayCastと強化学習の併用
# PROGRAM NAME : Reinforcement_RunA_RayCast_Maze.py
# step11B : カメラ画像の方向は、2DMazeの学習結果で決めています。量子化用の対応がされています
...
def calc_Boltzman(MazeXpos, MazeYpos, movablesMap, pos_lm):
    ...
    C_Target    = 5.0   # ボルツマン選択レベルの目標値
    arr_values  = C_Temp/(0.2*arr_Gdists**3+10*arr_sminRTA**2+1.0)
...
class DQN_Map_Solver3D:
    ...
    def __init__(self, stateMap_size, actionMap_size):
        ...
        self.epsilon    = 0.85 # randomness of choosing random actionMap or the best one
        self.e_decay    = 0.999  # epsilon decay rate
...
class Agent(object):
    ...
    def __init__(self, SCREEN):
        ...
        for iCnt_play in range(NUMS_EPISODES):
            ...
            # カリキュラム学習
            self.mx_lm        = [[0, 2],[20, 9]]   # ゴール　元は[26, 25]   [10, 9] -> [13, 9] -> [17, 9]
            self.arr_lmG      = self.mx_lm[1]
    ...
    def get_episode(self, iCnt_play):
        ...
        for iCnt_turn in range(NUMS_TURNS):
            ...
            if next_stateMaze_chess[0] >= self.arr_lmG[0]-2 and next_stateMaze_chess[1] >= self.arr_lmG[1]-2:
                ...
                # ----------------
                # 最終報酬の計算
                reward  = nominal_reward + 15000 - 500*int(end_distGoal) - 1000*int(end_distRTA) - sum_bump - 10*iCnt_play
                ...
                if iCnt_turn == NUMS_TURNS-1:
                    ...
                    doneMap     = True
                    # 最終報酬の計算
                    reward  = nominal_reward + 4000 - 500*int(end_distGoal) - 1000*int(end_distRTA) - sum_bump
                    print("sorry! no enoughgame time, reward:{}".format(reward))
                else:
                    # 中間段階
                    reward  = nominal_reward - 0.2*int(end_distGoal) - 0.7*int(end_distRTA) - 0.1*sum_bump
                # ----------------
                # [Map]データを積み上げる
...
if __name__ == '__main__':
    ...
    comment_output_3Dmodel = "secondB"
...
QEU：FOUNDER ; “**「転写」のポイントであるボルツマン選択の関数**に対してもう少し解説しておきましょうか・・・。”
...
QEU：FOUNDER ; “ボルツマン選択のポイントである**「ボルツマン選択レベルの目標値（C_Target）」**に関して、もう少し解説しておきましょうか・・・。”

![imageRL6-12-6](https://reinforce.github.io/images/imageRL6-12-6.jpg) 

QEU：FOUNDER ; “この温度の値を変えれば、項目を選択する確率を特定要素に集中するかをけってできます。つまり、「転写精度」をコントロールできるわけです。”

D先生 : “転写を強くすれば、計算は速いが関数の柔軟性は失われます。一方、転写を弱くすれば関数は柔軟になります。そうそう・・・、ディープラーニングの関数定義が変わったので、カリキュラム学習は最初からやり直しになったでしょう・・・。”

QEU：FOUNDER ; “それではやり直しの最初の「第1段階」の結果を見てみましょう。”

**（パフォーマンス推移A、第一段階）**

![imageRL6-12-7](https://reinforce.github.io/images/imageRL6-12-7.jpg) 

**（パフォーマンス推移B、第一段階）**

![imageRL6-12-8](https://reinforce.github.io/images/imageRL6-12-8.jpg) 

QEU：FOUNDER ; “あと、コレは「第二段階」の結果ね・・・。ホントに計算には時間がかかります。”

**（パフォーマンス推移A、第二段階-STEP1）**

![imageRL6-12-9](https://reinforce.github.io/images/imageRL6-12-9.jpg) 

**（パフォーマンス推移B、第二段階-STEP1）**

![imageRL6-12-10](https://reinforce.github.io/images/imageRL6-12-10.jpg) 

D先生 : “Step1はうまく行っています・・・。”

QEU：FOUNDER ; “ステップ３をドン！！”

**（パフォーマンス推移A、第二段階-STEP3）**

![imageRL6-12-11](https://reinforce.github.io/images/imageRL6-12-11.jpg) 


**（パフォーマンス推移B、第二段階-STEP3）**

![imageRL6-12-12](https://reinforce.github.io/images/imageRL6-12-12.jpg) 

D先生 : “あれ？他の場所に学習収束していません？”

QEU：FOUNDER ; “第3段階では**C_Target値を変更して、もうちょっと転写性を上げましょう**。”


## ～　まとめ　～

QEU:FOUNDER ： “D先生の大好きな**「大美人ジャーナリスト」**について・・・。”

![imageRL6-12-13](https://reinforce.github.io/images/imageRL6-12-13.jpg) 

D部長: “あれ？この人は、**FOUNDERご贔屓のエズ様本の翻訳監修者**じゃないですか・・・（笑）。へえ・・・。**「経済・外交・軍事」の必然のシナリオ**らしいです。”

![imageRL6-12-14](https://reinforce.github.io/images/imageRL6-12-14.jpg) 

QEU:FOUNDER ： “ともあれ、現実の結果がそうなっているからね。みんな仲良し・・・、ドン(笑)。”

![imageRL6-12-15](https://reinforce.github.io/images/imageRL6-12-15.jpg) 

D部長: “**「和をもって貴しとなす」**というではないですか・・・。”

QEU:FOUNDER ： “いやいや、すばらしい・・・。”
