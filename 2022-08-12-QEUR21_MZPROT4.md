## QEUR21_MZPROT4: 画像をメトリックス化して、メイズを歩く

## ～　やっと、強化学習の準備ができた・・・　～


QEU：FOUNDER ; “前回のプログラム開発で、**画像の「量子化(Quantization)」**が完了しました。”

![imageRL6-5-1](https://reinforce.github.io/images/imageRL6-5-1.jpg) 

D先生 : “次は、**画像のメトリックス化**ですね。どんな方針にします？さしあたり、青色と赤色を使うのはわかりました。”

QEU:FOUNDER ; “**画面を「左(L)-中央(C)-右(R)」に分けて、画素を数えればいい**と思います。これで**特徴量は6次元**になります。このメトリックスが使えるかどうかは、実際にメイズの中を歩いてみるしかないよね。それでは、プログラムをドン！！”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step4C : RayCastと強化学習の併用
# PROGRAM NAME : demonstration_cntlVisual_RayCast_Maze.py
# step4C : マニュアル操作版ですが、さらに量子化用の対応がされています
# step4C : カメラ画像の方向は、2DMazeの学習結果で決めています。
...
class Game(object):
    ...
    def __init__(self, SCREEN):
        ...
        while True:
            ...
            if FLG_KEY == "ON":
                # 回転と前進に翻訳してコマを動かす
                state, arr_RTDists, movables = self.move_chess(CRR_MPODR, iCnt_turn)
                # -----
                # pygameへの表示
                ...
                #print("-----  screen_pic_iStep:{}  -----".format(iStep))
                #print(dst_small)
                ...
                cc_L = Counter(board_blue_L)
                cc_C = Counter(board_blue_C)
                cc_R = Counter(board_blue_R)
                ...
                cc_L = Counter(board_red_L)
                cc_C = Counter(board_red_C)
                cc_R = Counter(board_red_R)
                ...
                # -------------------------
                # 状態表示
                print("iTurn:{}, CRR_MPODR:{}, state:{}, MapPosition: [{},{}], MapAngle:{}, ViewAngle:{}".format(iCnt_turn, CRR_MPODR, state, self.MapPosition[0], self.MapPosition[1], self.MapAngle, self.ViewAngle))
                # カウントアップ
    ...
    # ------------------------------------------------
    # コマを動かす
    def move_chess(self, CRR_MPODR, iCnt_turn):
        ...
        if CRR_MPODR == 'LEFT':
            self.MapAngle = 180
        elif CRR_MPODR == 'RIGHT':
            self.MapAngle = 0
        elif CRR_MPODR == 'UP':
            self.MapAngle = 270
        elif CRR_MPODR == 'DOWN':
            self.MapAngle = 90
        self.MapDirection = round(2*math.pi*self.MapAngle/360, 4)
        ...
        if (canGo(tmpX,tmpY,gameboard)):
            self.MapPosition[0] = round(tmpX, 4)
            self.MapPosition[1] = round(tmpY, 4)
        # -------------------------
        # メイズのXY座標
...
QEU：FOUNDER ; “それでは、まずは動作の状況から見てみましょう。”

![imageRL6-5-2](https://reinforce.github.io/images/imageRL6-5-2.jpg) 

D先生 : “青色用に3次元、赤色用に3次元のリストが出力されています・・・。”

QEU：FOUNDER ; “次にグラフを見てみましょう。”

![imageRL6-5-3](https://reinforce.github.io/images/imageRL6-5-3.jpg) 

D先生 : “なるほどね・・・。特に、赤のメトリックスの動きは我々の直感に合っていますね。”

QEU：FOUNDER ; “強化学習に使えるかな？”

D先生 : “そりゃあ・・・、使えるでしょう。なぜ、わざわざこのメトリックスを使うのかという疑問が残りますが・・・。”

QEU:FOUNDER ; “そこらへんの話は次回やりましょう。すいませ～ん。カンパをください！！”
...
QEU：FOUNDER ; “ハリウッド・スターが、またなんか言っているなぁ・・・。”
...
D部長: “結局、**「あの戦争はなんだったのか」**と思うよね・・・。”

![imageRL6-5-4](https://reinforce.github.io/images/imageRL6-5-4.jpg) 

QEU:FOUNDER ; “ひどい話だよね、誰が「ひどい」のかは敢えて言わないけど・・・（笑）。一体、誰が責任を取るの？”

<iframe width="560" height="315" src="https://www.youtube.com/embed/a3lG6iUKydM" ti-tle="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; en-crypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ; “ポリコレがあるから、長い間あえて言わなかったけど・・・。U国大統領って、なぜあんなにチヤホヤされたんだろう。逆に、チヤホヤした人も責任を取ってほしいよね・・・。”

![imageRL6-5-5](https://reinforce.github.io/images/imageRL6-5-5.jpg) 

D部長: “外部は口を出すが責任をとらない。これを**「オフショア・バランシング・ポリシー」**とよぶ・・・。インフレーションでツケを払うことになるのかな？”

![imageRL6-5-6](https://reinforce.github.io/images/imageRL6-5-6.jpg) 

QEU:FOUNDER ; “このインフレデータ面白いね。・・・なにはともあれ、もう一度この「飄々とした」お方の話をみてみましょう・・・。”

![imageRL6-5-7](https://reinforce.github.io/images/imageRL6-5-7.jpg) 

D部長: “**本当の保守とはだれか**を、もうそろそろ理解すべきじゃないでしょうか・・・。”
