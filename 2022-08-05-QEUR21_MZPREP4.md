## QEUR21_MZPREP4: Boltzman選択とDQN with Experience Replay

### ・・・　前回のつづきです　・・・

QEU：FOUNDER ; “残念ながらA3C(Actor-Critic系列)には収束させる力がなさそうだね。”

D先生 : “じゃあ、ε-Greedy系列に行きましょうか・・・。DQN with Experience Replayに・・・。”

QEU:FOUNDER ; “ただし、ε-Greedyをそのまま使うとハマるよ。Boltzmann行動選択の考え方を使いましょう。「ボルツ？なんだソレ？」を説明する前に、デモ用のプログラムをドン！！”
...
for i in range(nrows):
    for j in range(ncols):
        ...
        axes[i, j].bar(range(qs_prob.shape[0]), qs_prob)
        axes[i, j].set_title('temp = {}'.format(t))
plt.tight_layout()
plt.show()

```

![imageRL5-5-1](https://reinforce.github.io/images/imageRL5-5-1.jpg) 

D先生 : “当然、やっていることの意味がわからないですね。”

![imageRL5-5-2](https://reinforce.github.io/images/imageRL5-5-2.jpg) 

QEU:FOUNDER ; “ボルツマン方案選択法の定義はコレ（↑）だが、このスキームは値を確率に変換するという特性を持っているんです。つまり、次の行動を選択するために複数の方案が考えられる場合、その報酬に合わせてrandom.choice関数での選択確率が変わるわけ・・・。


D先生 : “なるほど・・・。**特に今回のメイズの場合には「正解（基準ルート）」がわかっているので、それを優先的に学習したいですよね。**”

![imageRL5-5-3](https://reinforce.github.io/images/imageRL5-5-3.jpg) 

QEU:FOUNDER ; “今回は距離を使って選択確率に変換しています。”

![imageRL5-5-4](https://reinforce.github.io/images/imageRL5-5-4.jpg) 

D先生 : “あと、なんか特記事項は？”

QEU：FOUNDER ; “**DQN with Experience Replay**を使ったので、強制学習が使えないなどA3Cとはカリキュラム学習のやり方が変わります。結論としては、**ずいぶん楽になった**…。プログラムをドン！！。”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step4C : 3Dメイズの準備段階です(DQNに方針変更)
# step4C : Reinforce_3DMazePrep_pyTorch(DQN_size27).py
# step4C : DQN-ERとsminRT距離ｘ3、さらにBoltzmann行動選択を導入する
# ---------------- 
# 数値計算のﾗｲﾌﾞﾗﾘ読み込み
import numpy as np
import pandas as pd
...
class Agent():
    ...
    def __init__(self):
        ...
        for iCnt_play in range(NUMS_EPISODES):
            ...
            # 学習結果の出力
            if iCnt_play % 2 == 0:
                print("iplay:{0}, stage:{1}, maxturn:{2}, maxscore:{3}, FGoal:{4}, Eps:{5}, loss:{6}, QVs:{7}".format(iCnt_play, stage_info, maxturn, maxscore, flag_goal, val_epsilon, val_loss, list_QVs))
    ...
    def get_episode(self, iCnt_play):
        ...
        for iCnt_turn in range(NUMS_TURNS - 1):
            ...
            # ----------------
            # ゴールに到達した
            if iCnt_play < 300 and next_state_chess[0] >= 15 and next_state_chess[1] >= 15:
                done = True
                reward = 10000 - 5*iCnt_turn
                nominal_reward = 10000
            elif iCnt_play >= 300 and iCnt_play < 600 and next_state_chess[0] >= 20 and next_state_chess[1] >= 20:
                done = True
                reward = 10000 - 5*iCnt_turn
                nominal_reward = 10000
            elif iCnt_play >= 600 and next_state_chess[0] >= 25 and next_state_chess[1] >= 25:
                done = True
                reward = 10000 - 5*iCnt_turn
                nominal_reward = 10000
            else:
...
QEU：FOUNDER ; “それでは、結果をドン・・・。”


**(学習曲線)**

![imageRL5-5-5](https://reinforce.github.io/images/imageRL5-5-5.jpg) 

D先生 : “ε（イプシロン）がおもしろい動きをしています。これが、例の**「カリキュラム学習」**ですね・・・。”

QEU：FOUNDER ; “今回のカリキュラム学習は、ゴールの位置を動かしています。それでは、パフォーマンス推移を見てみましょう。”


**(パフォーマンス推移)**

![imageRL5-5-6](https://reinforce.github.io/images/imageRL5-5-6.jpg) 

D先生 : “久しぶりに見た、うまい学習収束ですね・・・（笑）。時間がなくて、1500回しか学習しなかったんですね。ああ、残念・・・。”

QEU：FOUNDER ; “学習が足らないよね。でも、Q値の動きを見ると、うまく行きそうだという手ごたえがありますね。・・・、というわけで**大型メイズを最適化するための技術が提案できたわけ**です。”

C部長 : “・・・とすると、3Dメイズの本番に移行できますか？”

QEU:FOUNDER ; “まだまだ、やるべき準備事項があります・・・。次は、もうちょっとキツいかな・・・。すいませ～ん。カンパをください！！”
...
![imageRL5-5-7](https://reinforce.github.io/images/imageRL5-5-7.jpg) 
 
QEU:FOUNDER ： “イケメン、美人政治家が揃っていますね・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/UgVUAZRTGac" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

D先生 : “**「KOKUSO反対」の件**ですか・・・。”
 
QEU:FOUNDER ： “**「財政原理としてお金は刷れるが、こういうことに使うべきではない」**・・・。明解な考え方ですね。”
 
C部長: “**「お金が刷れるんだから、なんでもやれるんだ」って言う考え方はおかしいですね。やっぱり、政策の妥当性を主体にして議論しないと**・・・。市中の人も、ここまでの議論（↓）ができるんだし・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/CSIIVfXx3yo" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ; “この人たちは、ひょっとしたら**「後で偉くなる」**のかもしれません。”
