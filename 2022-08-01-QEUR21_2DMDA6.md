## QEUR21_2DMZA6:　補足　～  XY座標を再び採用する

## ～  次に行こう！！  ～

### ・・・　前回のつづきです　・・・

QEU:FOUNDER ： “（6-ランドマーク距離インプットの実行結果は）どうですか・・・？”

**（学習曲線）**

![imageRL4-15-1](https://reinforce.github.io/images/imageRL4-15-1.jpg) 

**（パフォーマンス推移）**

![imageRL4-15-2](https://reinforce.github.io/images/imageRL4-15-2.jpg) 

**（原因分析：なぜ学習の収束が悪いのか）**

- その１：インプット情報が不足している
- その２：インプット情報は十分だが合理的でなく、学習するのに手間がかかる
- その３：インプット情報が乱れており、学習できない

QEU:FOUNDER ： “それでは原因分析をしてみましょうか・・・。BUMP項目を導入する前は、インプット情報が乱れて学習できませんでした（その３）。その後、BUMP対策によりゆっくりと収束しているようなので、インプット情報が不足しているとはいえないです（その１）。つまり、今回の問題はインプット情報が合理的ではないことです。すなわち「座標系の問題」で、６-ランドマーク距離のインプットはよくないのでしょう。”

![imageRL4-15-3](https://reinforce.github.io/images/imageRL4-15-3.jpg) 

D先生 : “**余弦定理を使えば、ボード角部の座標が3点あればXY座標を計算することができます**からね・・・。実は、四隅のベンチマークは重複するんですよね。”

![imageRL4-15-4](https://reinforce.github.io/images/imageRL4-15-4.jpg) 

QEU:FOUNDER ： “今回は別に余弦定理を使わずとも、セルのアドレス（row,col）を使えばいいだけです。それではプログラムをドン！！”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# stepD4 : STATEをNON-ONEHOTと動的メトリックスで構成する
# stepD4 : reinforce_maze_pyTorch(XY-A3C-sminRT).py
# stepD4 : Bump項をSTATEに追加して、6-Landmarkの代わりに、もとのXY座標を入力にするとどうなるかをテストしました
# ---------------- 
# 数値計算のﾗｲﾌﾞﾗﾘ読み込み
import numpy as np
import pandas as pd
...
def calc_address(a_row, a_col, num_bump):
...
class Agent():
    def __init__(self):
        ...
        self.value_network_optim = torch.optim.Adam(self.value_network.parameters(), lr=0.0005)
    ...
    def roll_out(self, iCnt_play):
        ...
        num_bump    = 0
        state = calc_address(state_chess[0], state_chess[1], num_bump)
        # 移動稼働なオプション
        ...
        for iCnt_turn in range(SAMPLE_NUMS):
            ...
            # ----------------
            # 強制的に最適ルートを入力する
            if iCnt_play == 0 or iCnt_play == 2 or iCnt_play == 5 or iCnt_play == 8:
            ...
            next_state = calc_address(next_state_chess[0], next_state_chess[1], num_bump)
...
QEU:FOUNDER ： “今回も実行結果をみて終わりです。どうですか・・・？”

**（学習曲線）**

![imageRL4-15-5](https://reinforce.github.io/images/imageRL4-15-5.jpg) 

**（パフォーマンス推移）**

![imageRL4-15-6](https://reinforce.github.io/images/imageRL4-15-6.jpg) 

D先生 ： “前回に比較して、強化学習が相当にスムーズになっています。トータル5000回ぐらい繰り返し学習すれば、学習が完全に安定するのではないでしょうか。多分、ONEHOTの状態表現はいらないですね。”

QEU:FOUNDER ： “補足で延長した2Dメイズはここまでにしましょう。これから後の開発は3Dメイズ（前半）のシリーズでやりましょう。”

## ～　まとめ　～

QEU:FOUNDER ： “賢い人はいろんなことを勉強しているんだなぁ・・・。”

![imageRL4-15-7](https://reinforce.github.io/images/imageRL4-15-7.jpg) 

C部長: “なにか？えっ？「スチューデント・ワーク！？イエール！！？？」”

![imageRL4-15-8](https://reinforce.github.io/images/imageRL4-15-8.jpg) 

QEU:FOUNDER ： “よくわからんが、**日本の保守の研究をしている**らしいよ。論文の内容を読むのも面倒なので、参考文献に注目していました。”

![imageRL4-15-9](https://reinforce.github.io/images/imageRL4-15-9.jpg) 

D先生 : “なんか、**見覚えのある固有名詞**がいっぱい・・・（笑）。”

![imageRL4-15-10](https://reinforce.github.io/images/imageRL4-15-10.jpg) 

QEU:FOUNDER ： “このチャンネル、昔は面白かったんだけどね・・・。”

D先生 : “近現代史のシリーズとかね・・・”

![imageRL4-15-11](https://reinforce.github.io/images/imageRL4-15-11.jpg) 

QEU:FOUNDER ： “(小生は)高校のとき「世界史」を授業で取っていたので、日本史の内容はあまり知らないんです。このシリーズは切り口が斬新でおもしろかったなぁ・・・。”
