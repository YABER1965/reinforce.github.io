## QEUR21_MZPROT6: 閑話休題～バージョンアップ中 (トライアルB)

## ～　今回は完全に「メモ扱い」・・・　～

QEU:FOUNDER ; “今回は、前回のプログラムでCSV出力した命令データを読み込んで、自動的にゲームを走らせます。今回のプログラムにより、Agentクラスが生成されます。”

![imageRL6-7-1](https://reinforce.github.io/images/imageRL6-7-1.jpg) 

D先生 : “**「外枠だけ」**ながら、やっと強化学習らしくなりましたね・・・(笑)。”

![imageRL6-7-2](https://reinforce.github.io/images/imageRL6-7-2.jpg) 
...
# step7A : [3D]RayCastと強化学習の併用
# PROGRAM NAME : demonstration_trialB_RayCast_Maze.py
# step7A : マニュアル操作版ですが、さらに量子化用の対応がされています
# step7A : カメラ画像の方向は、2DMazeの学習結果で決めています。
# 注意： 散布図を使って、軌跡を表示しました。 軌跡データはCSVファイルに保管されています。(TRIAL_A)
#      命令(ACTION)群を使って、自動的に運航します。(TRIAL_B)
# ----------------
import cv2
import numpy as np
import pandas as pd
...
# -------------------------
# コマのトライアルDRルートのCSVファイルを読み込み表示する(TRIAL_B)
def read_DRchessfile():
    # CSVファイルの読み込み
    code_csvout = "maze_draw_route.csv"       # file name
    ...
    # -------------------------
    # 選択項目の読み込み
    mx_DRroute    = df.loc[:, "x":"y"].values.tolist()
    ref_DRaction  = df.loc[:, "action"].values.tolist()
    print("ref_DRaction: ", ref_DRaction)
    #print("----- route -----")
    #print(route)

    return mx_DRroute, ref_DRaction
...
#print(mx_routeC)
# ----------
# トライアル用DRルートを読み込み(TRIAL_B)
mx_DRroute, ref_DRaction = read_DRchessfile()
print(ref_DRaction)
print("--- mx_DRroute ---")
print(mx_DRroute)
...
class Agent(object):
    ...
    def __init__(self, SCREEN):
        ...
        #print("--- mx_DRroute ---")
        #print(mx_DRroute)
        NUMS_TURNS = len(ref_DRaction)

        # ゲームを実行する
        for iCnt_turn in range(NUMS_TURNS):

            # マップ(MP)コマンドを定義します
            actionMaze  = ref_DRaction[iCnt_turn]
            # キーがインプットされました
            time.sleep(0.3)
            # 回転と前進に翻訳してコマを動かす
...
QEU：FOUNDER ; “今回はメモ扱いなので、結果は紹介しないですよ。”
...
![imageRL6-7-3](https://reinforce.github.io/images/imageRL6-7-3.jpg) 

C部長 ： “なにかありますか？・・・・なければ、ボクから・・・・。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/gOuLa2sOF9s" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

D先生 ： “えっ？そんなことをするんだ・・・。”

QEU:FOUNDER ： “もらった人間（国）にメリットがあるのか?いわゆるメリットって、結局のところ「勝つこと」だけど・・・。もし勝てなければ、U国の人がより多くタヒぬだけだよ・・・。”

D先生 : “R国とのコネクションが切れたことが痛かったですね。戦争を止めることが最大の貢献のはずだから・・・。”
