## QEUR21_2DMZ5:　フィードバックによる学習加速（DQN with experience replay）

## ～　これが、本プロジェクトの成果「その一」　 ～

### ・・・　前回のつづきです　・・・

QEU:FOUNDER : “ライムラインRTはゲームの評価に使いましょう。つぎは、いよいよフィードバック制御です。タイムラインminRTメトリックスを使えば、フィードバックあるなしでのパフォーマンスの差異がわかりやすくなります。”

![imageRL4-6-1](https://reinforce.github.io/images/imageRL4-6-1.jpg) 

D先生 ： “それでは、フィードバックによる強化学習をやってみましょう。”

![imageRL4-6-2](https://reinforce.github.io/images/imageRL4-6-2.jpg) 

D先生 ： “なぜ、フィードバック係数を0.3に設定したんですか？”

QEU:FOUNDER : “「ノリ」です（笑）。それでは、プログラムをドン！！”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# step5B : RTを動的メトリックスとして活用する
# step5B_dqn_maze_pyTorch(space_minRT_FEEDBACK).py
# step5B : DQN-Experience replay
# ---------------- 
# 数値計算のﾗｲﾌﾞﾗﾘ読み込み
import numpy as np
import pandas as pd
...
class DQN_Solver:
    ...
    def choose_action(self, state, movables):
        ...
        if self.epsilon >= random.random():
            ...
            return acType, action, Qvalue, T_Dist, dist_sminRT
        else:
            # choosing the best action from model.predict()
            acType, action, Qvalue, T_Dist, dist_sminRT = self.choose_best_action(state, movables)
            return acType, action, Qvalue, T_Dist, dist_sminRT
    ...
    def choose_best_action(self, state, movables):
        ...
        for a in movables:
            ...
            input_state = calc_address(state[0], state[1])
        ...
        return acType, action, Qvalue, input_state[4]+input_state[5], input_state[6]
        ...
        for ibat in range(batch_size):
            ...
            if done:
                ...
                for a in next_movables:
                    ...
                    # ONEHOT状態(INPUT_STATE)を生成する
                    input_next_state = calc_address(next_state[0], next_state[1])
...
class Agent():
    ...
    def get_episode(self, iCnt_play):
        ...
        for time in range(times):
            ...
            acType, action, Qvalue, T_Dist, dist_sminRT = dql_solver.choose_action(state, movables)
            ...
            # ---------------------------
            # タイムライン用minRTメトリックスを計算する
            dist_tlminRT = timeline_minRT(state[0], state[1], time)
            # REWARD- FEEDBACK
            reward = reward - 0.3 * round(dist_sminRT,2)
...
QEU:FOUNDER : “それでは、学習結果をドン！！D先生、FBで学習パフォーマンスが良くなりましたか？“

**（学習曲線、FBなし）**

![imageRL4-6-3](https://reinforce.github.io/images/imageRL4-6-3.jpg) 

**（パフォーマンス推移、FBなし）**

![imageRL4-6-4](https://reinforce.github.io/images/imageRL4-6-4.jpg) 

**（学習曲線、FBあり）**

![imageRL4-6-5](https://reinforce.github.io/images/imageRL4-6-5.jpg) 

**（パフォーマンス推移、FBあり）**

![imageRL4-6-6](https://reinforce.github.io/images/imageRL4-6-6.jpg) 

D先生 ： “まあ、（パフォーマンスは）それなりに良くなったといえるでしょう。minRTメトリックスのグラフには、移動平均があればもっとわかりやすかったでしょうね。”

QEU:FOUNDER : “次回に改善します（笑）。これでDQN-with Experience replayでのFeedback効果の確認は終わりました。次は、REINFORCEでやってみましょう・・・。”

D先生 ： “前回やったことの確認だけですけどね・・・。”

QEU:FOUNDER : “REINFORCEは収束しにくいから、その点では効果がより明確にでるのではないでしょうか・・・。minRTは使いやすいからね・・・。”

## ～ まとめ ～

QEU:FOUNDER ： “なにがなんだか、よくわからない・・・。”

![imageRL4-6-7](https://reinforce.github.io/images/imageRL4-6-7.jpg) 

D先生 : “こんなツイートがでてきて、ひっくり返るほどおどろいたが・・・。”

![imageRL4-6-8](https://reinforce.github.io/images/imageRL4-6-8.jpg) 

D先生 : “さすがに、webで裏は取れなかった・・・。ホント・・・、変なモノを出すなよな・・・。この点では、このメディアは良心的です。”

<iframe width="560" height="315" src="https://www.youtube.com/embed/GswNM9R_1to" ti-tle="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>

QEU:FOUNDER ： “人間が出て来て配信している場合には、ごまかしがききませんからね。あと・・・、既存メディアも「本気を出せば」、それなりに信用できるし・・・。”

![imageRL4-6-9](https://reinforce.github.io/images/imageRL4-6-9.jpg) 

D先生 : “〇〇県ねえ・・・。”

QEU:FOUNDER ： “前回のネタと一緒に見ると、味わいが深い。”

![imageRL4-6-10](https://reinforce.github.io/images/imageRL4-6-10.jpg) 

QEU:FOUNDER ： “保守ってマジメなんだよね。西部先生みたいに・・・。”
...
D先生 : “FOUNDERは、あの人の一件で大ショックを受けてましたね。”

QEU:FOUNDER ： “そりゃそうだよ。・・・でもね、あの先生って、世の中が保守ブームなのにドンドン悲観的になったんだよね。「もうダメだ・・・」って・・・。”

D先生 : “最後はゼミナールで「ある意味で社会主義的なシステムが必要だ」って言っていました。たしか・・・。”
