## QEUR21_2DMZA5:　補足　～　６つのランドマーク(LANDMARK)を再び採用する

## ～  う～ん・・・。もう一声！！  ～

QEU:FOUNDER ： “今回の解説は簡単に行こう。ＯＮＥＨＯＴの代わりに6つのメトリックスを使って次元削減しましょう。プラスRTメトリックスを使ってね・・・。”

![imageRL4-14-1](https://reinforce.github.io/images/imageRL4-14-1.png) 

D先生 : “FOUNDER・・・。BUMP変数も追加してください・・・。”

![imageRL4-14-2](https://reinforce.github.io/images/imageRL4-14-2.png) 

QEU:FOUNDER ： “あとはどうでもいいや・・・。それではプログラムをドン！！”

```python
# ----------------
# メイズ(MAZE)ゲームの強化学習システム
# stepD3 : STATEをNON-ONEHOTと動的メトリックスで構成する
# stepD3 : reinforce_maze_pyTorch(6Landmark-A3C-sminRT).py
# stepD3 : Bump項をSTATEに追加して、6-Landmarkにするとどうなるかをテストしました
# ---------------- 
# 数値計算のﾗｲﾌﾞﾗﾘ読み込み
import numpy as np
import pandas as pd
...
def calc_address(a_row, a_col):
    ...
    state = np.hstack([state, [dist_sminRT]])
...
class Agent():
    ...
    def roll_out(self, iCnt_play):
        ...
        for iCnt_turn in range(SAMPLE_NUMS):
            ...
            # ----------------
            # 強制的に最適ルートを入力する
            if iCnt_play == 0 or iCnt_play == 2 or iCnt_play == 5 or iCnt_play == 8 or iCnt_play == 1501 or iCnt_play == 1504 or iCnt_play == 1507:
...
QEU:FOUNDER ： “今回は特にいうことはないんで、実行結果をみて終わりです。どうですか・・・？”

**（学習曲線）**

![imageRL4-14-3](https://reinforce.github.io/images/imageRL4-14-3.png) 

**（パフォーマンス推移）**

![imageRL4-14-4](https://reinforce.github.io/images/imageRL4-14-4.png) 

D先生 ： “惜しい・・・。基本、かなり（収束が）良くなっていますが、もうちょっと何とかならないですかね。”
...
QEU:FOUNDER ： “彼らには（立場を）変えられない事情があるんでしょうね。ひょっとしたら「なんかの宗教」とかも絡むのかな？”
...
QEU:FOUNDER ： “ハァ？野次馬ねえ・・・。いろんな意味で、（J国は）近いうちに相当ひどくなると思うよ。”

D先生 : “すいません。タイトルが「向こうの言葉」で、私にはよくわかりません・・・。”

![imageRL4-14-5](https://reinforce.github.io/images/imageRL4-14-5.png) 

QEU:FOUNDER ： “D先生のせっかくのリクエストなのでタイトルを翻訳しましょう・・・。ドン！！”

D先生 : “（タイトルの意味を知って）キャー！！”

![imageRL4-14-6](https://reinforce.github.io/images/imageRL4-14-6.png) 

QEU:FOUNDER ： “現在の状況からみて、今後の経済が悪くなるというのは誰にでもいえますが・・・。”

![imageRL4-14-7](https://reinforce.github.io/images/imageRL4-14-7.png) 

QEU:FOUNDER ： “2023年にいかなくても、すでに**「はやり病の件」は当たる確率は高い**よね・・・。この予言を当てる人は多くないでしょうね。”

D先生 : “なんとか回避できないですかね？せめて私だけでも・・・。”

QEU:FOUNDER ： “(あの風水先生に)聞いてみれば？でも、第三者のウォッチャー（我々）としてみれば、ほとんどの問題が**「（J国が）自らが招いている」**感じでしょ？いくら、この風水師が優秀でも、果たしてなんとかなるのかなぁ・・・。”
