...
QEU：FOUNDER ; “あまりにも遅くてシビれた（笑）！！でも、Qテーブルの計算結果をインポートできればトータルではPyTorchよりも早くなるかもね。あとは簡単にPythonコードをドン！！”
...
def calc_address(a_row, a_col, mx_lm):
    ...
    # --------------------------------------------------
    # STATEを生成する
    state   = np.array([add_state[1], dist_minRTA, dist_minRTB, dist_minRTC])
...
class DQN_Solver:
    def __init__(self, state_size, action_size):
        ...
        self.memory   = deque(maxlen=4000) # memory space
        self.gamma    = 0.98 # discount rate
        self.epsilon  = 0.85 # randomness of choosing random action or the best one
    ...
    def _build_model(self):
        model = Sequential() 
        model.add(Dense(512, activation="relu", input_dim=dim_input))
    ...
    def replay_experience(self, batch_size, iCnt_play):
        ...
        for ibat in range(batch_size):
            ...
            # ----------------
            # 状態(STATE)行動(ACTION)ベクトルを生成する
            state_action_curr = np.hstack([state, [diff_row, diff_col]]).tolist()
            ...
            if len(next_movables) > 0:          
                ...
                for a_order in next_movables:
                    ...
                    # ----------------
                    # 状態(STATE)行動(ACTION)ベクトルを生成する
                    np_next_s_a  = np.hstack([next_state, [diff_row, diff_col]]).tolist()
                    if iCnt == 0:
                ...
                y_pred      = self.model.predict(mx_input)
                #print("y_pred: ", y_pred.flatten())
                #idx_nr_max  = np.argmax(y_pred.flatten())
...
class Agent():
    ...
    def __init__(self):
        ...
        for iCnt_play in range(NUMS_EPISODES):
            ...
            val_epsilon, val_loss = dql_solver.replay_experience(BATCH_SIZE, iCnt_play)
            # QValueリストを生成する
    ...
    def get_episode(self, iCnt_play):
        ...
        for iCnt_turn in range(NUMS_TURNS - 1):
            ...
            if iCnt_play == 500:
                ...
                dql_solver.epsilon = 0.85
            elif iCnt_play == 1000:
                self.stage_info    = "stage3"
                dql_solver.epsilon = 0.80
            # ----------------
            # カリキュラム学習(2) ; 報酬
            if iCnt_play < 500 and next_state_chess[0] >= 15 and next_state_chess[1] >= 15:
                ...
                # [重要]フィードバック制御あり
                #diff_minRTB = dist_minRTB
                #diff_minRTC = dist_minRTC
                #if diff_minRTB > 0:
                #    diff_minRTB = 0
                #if diff_minRTC > 0:
                #    diff_minRTC = 0
                #reward = temp_reward + 30 * diff_minRTB + 30 * diff_minRTC - 20 * dist_minRT
                reward          = temp_reward
...
if __name__ == '__main__':
    ...
    code_output_model = "model_maze2D_DynaQ_{0}.h5".format(comment_output_model)  # モデルの指定
...
D先生 ; “収束は一応しているが・・・。Kerasの場合にはPyTorchの場合と収束の状況が変わっているように思います。”
